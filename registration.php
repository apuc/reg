...
function add_reg_style(){
    ...
    wp_enqueue_style( 'ui-style', REG_URL . 'css/jquery-ui.min.css', array(), '1');
    wp_enqueue_style( 'fuelux-style', REG_URL . 'css/fuelux.css', array(), '1');
    wp_enqueue_style( 'main-style', REG_URL . 'css/style.css', array(), '1');
    wp_enqueue_style( 'tabs-style', REG_URL . 'css/tabs.css', array(), '1');
}
...
function add_reg_script(){
    ...
    wp_enqueue_script( 'reg_script', REG_URL.'js/script.js', array(), '1');
    wp_localize_script( 'reg_script', 'myajax', array('url' => admin_url('admin-ajax.php')));
}
...
function reg_form(){
    ...
    if(isset($_POST['lastName'])){
        ...
        $user_id = reg_user($_POST);
        reg_user_meta($user_id, $_POST);
        $parser->parse(REG_DIR."/views/reg_success.php", array('id'=>$user_id), true);
    }
    else {
        $data['step1'] = $parser->parse(REG_DIR."views/step1.php", array(), false);
...
function get_country_field(){
    if($_POST['country'] == '0'){
        ...
        $parser = new ParserReg();
        $parser->parse(REG_DIR."views/country_fields/".$country.".php", array(), true);
    }
    die();
}
